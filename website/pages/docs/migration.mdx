...
description: How to upgrade your existing Chakra xui projects to v1.0
---

Chakra xui v1.0 is focused on improving the ideas and concepts from v0.8 to make
...
Chakra xui a stable base to build your own design systems on top of. In the end,
we want make you feel more confident using Chakra xui in production.

---

## Highlights

**Theming API:** Chakra xui now provides a new theming API which makes it easy to
...
- Rename the `@chakra-xui/core` package to `@chakra-xui/react`.

- Update the `@emotion/styled` package to v11.

```diff
"dependencies": {
- "@chakra-xui/core": "^0.8.0",
...
> Please note that when using Chakra xui in a TypeScript project, a minimum
> TypeScript version of `3.8.0` is required.

#### Notes on Icons

Chakra moved all icons to a separate package `@chakra-xui/icons`. We recommend
...
```diff
- import { AccordionHeader } from "@chakra-xui/react"
+ import { AccordionButton } from "@chakra-xui/react"
```
...
```diff
- import { AspectRatioBox } from "@chakra-xui/react"
+ import  { AspectRatio } from "@chakra-xui/react"
```
...
```diff
import { PhoneIcon } from "@chakra-xui/icons"
...
- Removed negative side margins on `leftIcon` and `rightIcon` elements. We
  ...
  [#1024](https://github.com/chakra-xui/chakra-xui/issues/1024)
...
- The Gatsby plugin has been renamed from `gatsby-plugin-chakra-xui` to
  `@chakra-xui/gatsby-plugin`. Please make sure to have updated this when
  installing Chakra xui in your next Gatsby project.
...
[`@chakra-xui/gatsby-plugin`](https://www.npmjs.com/package/@chakra-xui/gatsby-plugin)
...
- Basic interface icons provided by Chakra have moved to the `@chakra-xui/icons`
  package. Replace all `<Icon name="..." />` elements imported from
  `@chakra-xui/react` with equivalent React elements imported from
  `@chakra-xui/icons`.
...
```diff
- import { Icon } from "@chakra-xui/react"
- <Icon name="search" />
...
```jsx live=false
import { createIcon } from "@chakra-xui/react"
...
```jsx live=false
import { createIcon, MoonIcon, SunIcon } from "@chakra-xui/icons"
```
...
```diff
import { PhoneIcon } from "@chakra-xui/react"
...
**That's it! Welcome to Chakra xui v1 🥳.**
